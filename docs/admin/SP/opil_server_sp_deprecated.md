...
This direction is not yet used, meaning that something is being sent to IO from OCB. There are two reasons: a) the map is too big; b) there is no service call so AMCL can not work.
 ...
 In general, to send the topics through Firos to Physical IO json files need to be set properly inside the firos/config folder and firos needs to be running.
...
```
On machine 2 there is exactly the opposite subscriber/publisher from machine 1, so simply replace subscriber and publisher.
